  ...
  if (pid == 0) {
    ...
    if (res == MAP_FAILED) {
      result_or_error = ErrorMessage(absl::StrFormat(
          "Failed to execute mmap syscall with parameters address=%#x size=%u prot=PROT_WRITE: %d",
          address, size, errno));
    } else {
      result_or_error = (uint64_t)res;
    }
  } else {
    constexpr uint64_t kSyscallNumberMmap = 9;
    result_or_error = SyscallInTracee(pid, kSyscallNumberMmap, address, size, PROT_WRITE,
    ...
    if (auto res = mprotect((void*)address_, size_, PROT_EXEC | PROT_WRITE); res != 0) {
      return ErrorMessage(
          absl::StrFormat("Failed to execute mprotect syscall with parameters address=%#x size=%u "
                          "prot=PROT_EXEC: %d",
                          address_, size_, errno));
    }
  } else {
    constexpr uint64_t kSyscallNumberMprotect = 10;
    auto result_or_error =
        SyscallInTracee(pid_, kSyscallNumberMprotect, address_, size_, PROT_EXEC | PROT_WRITE, 0, 0, 0, 0);
    if (result_or_error.has_error()) {
      return ErrorMessage(
          absl::StrFormat("Failed to execute mprotect syscall with parameters address=%#x size=%u "
                          "prot=PROT_EXEC: %s",
