...
#ifdef TEST_NS
constexpr uint64_t kOffsetOfFunctionIdInCallToEntryPayload = 44;
#else
constexpr uint64_t kOffsetOfFunctionIdInCallToEntryPayload = 34;
#endif
...
void AppendBackupCode(MachineCode& trampoline) {
  ...
  trampoline.AppendBytes({0x53})
      ...
      .AppendBytes({0x41, 0x55})
      .AppendBytes({0x41, 0x56})
      .AppendBytes({0x41, 0x57});
#endif
...
void AppendCallToEntryPayload(uint64_t entry_payload_function_address,
...
#ifdef TEST_NS
  constexpr auto rsp_offset = 0x78;
#else
  constexpr auto rsp_offset = 0x48;
#endif
  ...
  // call rax                                        ff d0
  trampoline.AppendBytes({0x48, 0x83, 0xc0, rsp_offset})
...
#ifdef TEST_NS
  trampoline.AppendBytes({0x48, 0x83, 0xc4, 0x30});
#endif
