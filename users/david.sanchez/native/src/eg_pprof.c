...
printf("Map IDs: %lu, %lu\n", id_map0, id_map1);
  ...
  uint64_t id_fun0 = pprof_funAdd(dp, "recursion_fun", "recursion_fun", "Hello.c", 0);
  uint64_t id_fun1 = pprof_funAdd(dp, "work_fun", "work_fun", "t3st.c", 0);
printf("Fun IDs: %lu, %lu\n", id_fun0, id_fun1);

  // Add some fake locations
  uint64_t id_loc0 = pprof_locAdd(dp, id_map0, 50, (uint64_t[]){id_fun0}, (int64_t[]){0}, 1);
  uint64_t id_loc1 = pprof_locAdd(dp, id_map1, 250, (uint64_t[]){id_fun1}, (int64_t[]){0}, 1);
printf("Loc IDs: %lu, %lu\n", id_loc0, id_loc1);

  // Add some fake samples
  pprof_sampleAdd(dp, (int64_t[]){1,300}, 2, (uint64_t[]){id_loc0}, 1);
  pprof_sampleAdd(dp, (int64_t[]){1,300}, 2, (uint64_t[]){id_loc0, id_loc1}, 2);
  pprof_sampleAdd(dp, (int64_t[]){1,200}, 2, (uint64_t[]){id_loc0, id_loc1}, 2);
  pprof_sampleAdd(dp, (int64_t[]){1,100}, 2, (uint64_t[]){id_loc0, id_loc1}, 2);
