...
#define rmb() __asm__ volatile("lfence":::"memory") // TIL "rmb" is an lfence
...
#define PS_X_READ         struct {...}; \\ TODO
#define PS_X_CALLCHAIN    uint64_t nr; uint64_t ips[1];
#define PS_X_RAW          uint32_t size; char data[1];
#define PS_X_BRANCH_STACK uint64_t bnr; struct perf_brancH_entry lbr[1]; // TODO
#define PS_X_REGS_USER    uint64_t abi; uint64_t regs[0];
#define PS_X_STACK_USER   uint64_t size; char data[0]; uint64_t dyn_size;
#define PS_X_WEIGHT       uint64_t weight;
#define PS_X_DATA_SRC     uint64_t data_src;
#define PS_X_TRANSACTION  uint64_t transaction;
#define PS_X_REGS_INTR    uint64_t abi; uint64_t regs[0];
...
struct perf_event_attr g_dd_native_attr = {
    ...
    .config         = PERF_COUNT_SW_TASK_CLOCK,  // If it's good enough for perf(1), it's good enough for me!
...
char perfopen(pid_t pid, PEvent* pe, struct perf_event_attr* attr) {
  ...
  pe->fd = syscall(__NR_perf_event_open, attr, pid, 0, -1, PERF_FLAG_FD_CLOEXEC);
  ...
  // OK, now populate the page.  TODO what happens when hugepages is enabled, though?
  pe->region = mmap(NULL, PAGE_SIZE+PSAMPLE_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, pe->fd, 0);
...
void main_loop(PEvent *pe, void (*event_callback)(struct perf_event_header*, void*), void* callback_arg) {
  struct pollfd pfd = {.fd=pe->fd, .events=POLLIN | POLLERR | POLLHUP};
  if(!event_callback) event_callback = default_callback;
  ioctl(pe->fd, PERF_EVENT_IOC_ENABLE, 1);
  while(1) {
