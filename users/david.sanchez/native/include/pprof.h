...
uint64_t pprof_mapNew(DProf*, uint64_t, uint64_t, uint64_t, uint64_t, int64_t);
uint64_t pprof_funNew(DProf*, int64_t, int64_t, int64_t, int64_t);
uint64_t pprof_locNew(DProf*, uint64_t, uint64_t, uint64_t*, int64_t*, size_t);
uint64_t pprof_lineNew(DProf*, PPLocation*, uint64_t, int64_t);
char     pprof_sampleNew(DProf*, int64_t, uint64_t*, size_t, pid_t);
...
uint64_t pprof_mapNew(DProf* dp, uint64_t map_start, uint64_t map_end, uint64_t map_off, uint64_t id_filename, int64_t build) {
...
uint64_t pprof_mapAdd(DProf* dp, uint64_t map_start, uint64_t map_end, uint64_t map_off, char* filename, char* build) {
  ...
  for(size_t i=0; i < pprof->n_mapping; i++)
    if(isEqualMapping(map_start, map_end, map_off, id_filename, id_build, pprof->mapping[i]))
      return i+1;
  return pprof_mapNew(dp, map_start, map_end, map_off, id_filename, id_build)+1;
}

uint64_t pprof_lineNew(DProf* dp, PPLocation* loc, uint64_t id_function, int64_t line) {
UNUSED(dp);
...
uint64_t pprof_lineAdd(DProf* dp, PPLocation* loc, uint64_t id_function, int64_t line) {
  ...
  return pprof_lineNew(dp, loc, id_function, line)+1;
}
...
uint64_t pprof_funNew(DProf* dp, int64_t id_name, int64_t id_system_name, int64_t id_filename, int64_t start_line) {
  PPProfile* pprof = &dp->pprof;
...
uint64_t pprof_funAdd(DProf* dp, char* name, char* system_name, char* filename, int64_t start_line) {
  ...
  for (size_t i = 0; i < pprof->n_function; i++)
    if (isEqualFunction(id_name, id_system_name, id_filename, pprof->function[i]))
      return i+1;
  return pprof_funNew(dp, id_name, id_system_name, id_filename, start_line)+1;
}
...
uint64_t pprof_locNew(DProf* dp, uint64_t id_mapping, uint64_t addr, uint64_t* functions, int64_t* lines, size_t n_functions) {
  PPProfile* pprof = &dp->pprof;
...
uint64_t pprof_locAdd(DProf* dp, uint64_t id_mapping, uint64_t addr, uint64_t* functions, int64_t* lines, size_t n_functions) {
  ...
  return pprof_locNew(dp, id_mapping, addr, functions, lines, n_functions) + 1;
}
