...
TEST(ddprof_cpumask, nprocessors_conf) {
  int ncpus = ddprof::nprocessors_conf();
  cpu_set_t cpus;
  CPU_ZERO(&cpus);
  CPU_SET(0, &cpus);
  sched_setaffinity(0, sizeof(cpu_set_t), &cpus);
  // nprocessors_conf should not depend on affinity
  ASSERT_EQ(ncpus, ddprof::nprocessors_conf());
}

TEST(ddprof_cpumask, parse_cpu_mask) {
  cpu_set_t cpus;
  ASSERT_TRUE(ddprof::parse_cpu_mask("0x1", cpus));
  ASSERT_EQ(CPU_COUNT(&cpus), 1);
  ASSERT_TRUE(CPU_ISSET(0, &cpus));
  ASSERT_TRUE(ddprof::parse_cpu_mask("1", cpus));
  ASSERT_EQ(CPU_COUNT(&cpus), 1);
  ASSERT_TRUE(CPU_ISSET(0, &cpus));
  ASSERT_TRUE(ddprof::parse_cpu_mask("10", cpus));
  ASSERT_EQ(CPU_COUNT(&cpus), 1);
  ASSERT_TRUE(CPU_ISSET(4, &cpus));
  ASSERT_TRUE(ddprof::parse_cpu_mask("f", cpus));
  ...
  ASSERT_TRUE(CPU_ISSET(2, &cpus));
  ASSERT_TRUE(CPU_ISSET(3, &cpus));
  ASSERT_TRUE(ddprof::parse_cpu_mask("100000000", cpus));
  ASSERT_EQ(CPU_COUNT(&cpus), 1);
  ASSERT_TRUE(CPU_ISSET(32, &cpus));
  ASSERT_TRUE(ddprof::parse_cpu_mask("1,00000000", cpus));
  ASSERT_EQ(CPU_COUNT(&cpus), 1);
  ASSERT_TRUE(CPU_ISSET(32, &cpus));
}
