...
DDPROF_NOINLINE size_t func_save(std::span<std::byte> stack,
                                 std::span<uint64_t, PERF_REGS_COUNT> regs);
DDPROF_NOINLINE size_t
func_intermediate_1(int i, std::span<std::byte> stack,
                    std::span<uint64_t, PERF_REGS_COUNT> regs);

DDPROF_NOINLINE size_t func_save(ddprof::span<std::byte> stack,
...
static void BM_UnwindSameStack(benchmark::State &state) {
  ...
  int cpt = 0;
  for (auto _ : state) {
    // regs are over writen by async profiler
