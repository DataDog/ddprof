...
DDRes ddprof_exporter_init(const ExporterInput *exporter_input,
  ...
  exporter = std::make_shared<DDProfExporter>();
  ...
  exporter->_export = is_yes(exporter->_input.do_export);
...
DDRes ddprof_exporter_new(const UserTags *user_tags, DDProfExporter &exporter) {
  ...
  ddog_CharSlice base_url = to_CharSlice(exporter._url.c_str());
