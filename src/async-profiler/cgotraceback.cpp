...
// for benchmarking
void async_cgo_traceback_internal_set_enabled(int value) { enabled = value; }
...
static void cgo_context_release(struct cgo_context *c) { c->inuse = 0; }
...
static void truncate_asmcgocall(void **stack, int size) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    a += asmcgocall_base;
    if ((a >= (uintptr_t)asmcgocall_bounds->_start) &&
        (a <= (uintptr_t)asmcgocall_bounds->_end)) {
      if ((i + 1) < size) {
