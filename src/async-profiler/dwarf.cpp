...
void DwarfParser::parse(const char* eh_frame_hdr, u64 adjust_eh_frame) {
    ...
    if (version != 1 || (eh_frame_ptr_enc & 0x7) != 0x3 || (fde_count_enc & 0x7) != 0x3 || (table_enc & 0xf7) != 0x33) {
        LG_WRN("Unsupported .eh_frame_hdr [%02x%02x%02x%02x] in %s",
                  version, eh_frame_ptr_enc, fde_count_enc, table_enc, _name);
        return;
    }

    int fde_count = *(int*)(eh_frame_hdr + 8);
    int* table =  (int*)(eh_frame_hdr + 16) - adjust_eh_frame;
            ...
            case 0:
                switch (op) {
                    ...
                    case DW_CFA_GNU_args_size:
                        ...
                        break;
                    default:
                        LG_WRN("Unknown DWARF instruction 0x%x in %s", op, _name);
                        return;
                }
                break;
    ...
    while (_ptr < end) {
        ...
        switch (op) {
            ...
            case DW_OP_plus:
                ...
                break;
            default:
                LG_WRN("Unknown DWARF opcode 0x%x in %s", op, _name);
                _ptr = end;
                return 0;
        }
    }
